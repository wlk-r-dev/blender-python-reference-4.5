...
A common example is that exporters may access a mesh through `obj.data` (a [`bpy.types.Mesh`](bpy.types.Mesh.html#bpy.types.Mesh "bpy.types.Mesh")) when the user is in Edit-Mode, where the mesh data is available but out of sync with the edit mesh.
...
Using the [`bmesh`](bmesh.html#module-bmesh "bmesh") API is completely separate API from `bpy`, typically you would use one or the other based on the level of editing needed, not simply for a different way to access faces.
