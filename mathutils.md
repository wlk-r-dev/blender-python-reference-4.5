...
_class _mathutils.Euler(_angles_ , _order ='XYZ'_)
...
Parameters:
  ...
  * **order** (_str_) – Optional order of the angles, a permutation of `XYZ`.
...
Parameters:
  ...
  * **axis** (_str_) – single character in [‘X, ‘Y’, ‘Z’].
...
_class _mathutils.Matrix([_rows_])
...
**rows** (_Sequence_ _[__Sequence_ _[__float_ _]__]_) – Sequence of rows. When omitted, a 4x4 identity matrix is constructed.
...
_classmethod _Diagonal(_vector_)
...
_classmethod _LocRotScale(_location_ , _rotation_ , _scale_)
...
_classmethod _OrthoProjection(_axis_ , _size_)
...
_classmethod _Rotation(_angle_ , _size_ , _axis_)
...
_classmethod _Scale(_factor_ , _size_ , _axis_)
...
_classmethod _Shear(_plane_ , _size_ , _factor_)
...
Parameters:
  ...
  * **plane** (_str_) – Can be any of the following: [‘X’, ‘Y’, ‘XY’, ‘XZ’, ‘YZ’], where a single axis is for a 2D matrix only.
...
_classmethod _Translation(_vector_)
...
to_euler(_order_ , _euler_compat_)
...
Parameters:
  ...
  * **order** (_str_) – Optional rotation order argument in [‘XYZ’, ‘XZY’, ‘YXZ’, ‘YZX’, ‘ZXY’, ‘ZYX’].
...
_class _mathutils.Quaternion([_seq_[, _angle_]])
...
**axis** (_str_) – Twist axis as a string in [‘X’, ‘Y’, ‘Z’].
...
_class _mathutils.Vector(_seq_)
...
_classmethod _Fill(_size_ , _fill =0.0_)
...
_classmethod _Linspace(_start_ , _stop_ , _size_)
...
_classmethod _Range(_start_ , _stop_ , _step =1_)
...
_classmethod _Repeat(_vector_ , _size_)
...
angle_signed(_other_ , _fallback =None_)
...
slerp(_other_ , _factor_ , _fallback =None_)
...
Parameters:
  ...
  * **track** (_str_) – Track axis in [‘X’, ‘Y’, ‘Z’, ‘-X’, ‘-Y’, ‘-Z’].

  * **up** (_str_) – Up axis in [‘X’, ‘Y’, ‘Z’].
