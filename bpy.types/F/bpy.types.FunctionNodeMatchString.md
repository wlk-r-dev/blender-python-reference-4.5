...
_class _bpy.types.FunctionNodeMatchString(_FunctionNode_)
    

operation
    

  * `STARTS_WITH` Starts With – True when the first input starts with the second.

  * `ENDS_WITH` Ends With – True when the first input ends with the second.

  * `CONTAINS` Contains – True when the first input contains the second as a substring.
