...
_class _bpy.types.GeometryNodeResampleCurve(_GeometryNode_)
...
How to specify the amount of samples
  ...
  * `EVALUATED` Evaluated – Output the input spline’s evaluated points, based on the resolution attribute for NURBS and Bézier splines. Poly splines are unchanged.

  * `COUNT` Count – Sample the specified number of points along each spline.

  * `LENGTH` Length – Calculate the number of samples by splitting each spline into segments with the specified length.
