...
_class _bpy.types.GeometryNodeRemoveAttribute(_GeometryNode_)
...
How the attributes to remove are chosen
  ...
  * `EXACT` Exact – Remove the one attribute with the given name.

  * `WILDCARD` Wildcard – Remove all attributes that match the pattern which is allowed to contain a single wildcard (*).
