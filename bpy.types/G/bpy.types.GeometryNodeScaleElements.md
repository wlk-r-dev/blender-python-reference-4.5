...
_class _bpy.types.GeometryNodeScaleElements(_GeometryNode_)
...
Element type to transform
  ...
  * `EDGE` Edge – Scale individual edges or neighboring edge islands.
...
scale_mode
  ...
  * `UNIFORM` Uniform – Scale elements by the same factor in every direction.

  * `SINGLE_AXIS` Single Axis – Scale elements in a single direction.
