...
Menus have their [`UILayout.operator_context`](bpy.types.UILayout.md#bpy.types.UILayout.operator_context "bpy.types.UILayout.operator_context") initialized as ‘EXEC_REGION_WIN’ rather than ‘INVOKE_REGION_WIN’ (see [Execution Context](bpy.ops.md#operator-execution-context)). If the operator context needs to initialize inputs from the [`Operator.invoke`](bpy.types.Operator.md#bpy.types.Operator.invoke "bpy.types.Operator.invoke") function, then this needs to be explicitly set. When a menu is added to UI elements such as a panel or header, the operator execution context will be inherited from them.
...
path_menu(_searchpaths_ , _operator_ , _*_ , _props_default =None_, _prop_filepath ='filepath'_, _filter_ext =None_, _filter_path =None_, _display_name =None_, _add_operator =None_, _add_operator_props =None_)
